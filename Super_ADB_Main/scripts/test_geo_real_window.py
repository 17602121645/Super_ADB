...
MAIN = os.path.join(os.path.dirname(__file__), "..")  # -> Super_ADB_Main
sys.path.insert(0, os.path.abspath(MAIN))

# 用独立临时配置文件，绝不碰真实 adb_shell_config.json
_TMP = tempfile.mkdtemp(prefix="superadb_geo_")
TEMP_CONFIG = os.path.join(_TMP, "adb_shell_config.json")
open(TEMP_CONFIG, "w").write("{}")
...
# 让 Super_ADB_Main 模块读到我们自己的临时配置
import adb_utils
...
def _load_main():
    ...
    spec = importlib.util.spec_from_file_location(
        "Super_ADB_Main_geo", os.path.join(MAIN, "Super_ADB_Main.py"))
