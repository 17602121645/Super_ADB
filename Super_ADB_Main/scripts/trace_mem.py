...
4. 分阶段测量：
   S0 基线（仅最小 import）
   S1 导入 PySide6 核心
   S2 导入 Super_ADB_Main（此时重型子模块不应被加载 = 延迟 import 生效）
...
----
    ...
    D:/Python/Python314/python.exe Super_ADB_Main/scripts/trace_mem.py
...
# 让 `import Super_ADB_Main` 能找到 Super_ADB_Main/Super_ADB_Main.py
# （该目录下没有 __init__.py，必须把它自身加入 sys.path）。
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_MAIN_DIR = os.path.abspath(os.path.join(_SCRIPT_DIR, ".."))  # -> Super_ADB_Main/
...
# --------------------------------------------------------------------------- #
HEAVY_MODULES = [
    ("device_perf_monitor", "DevicePerfMonitor"),
    ("monkey_runner_window", "MonkeyRunnerWindow"),
    ("app_perf_monitor", "AppPerfMonitor"),
    ("install_zip_dialog", "InstallZipDialog"),
    ("tcpdump_dialog", "TcpdumpDialog"),
    ("about_dialog", "AboutDialog"),
    ("json_tool_dialog", "JsonToolDialog"),
    ("md5_dialog", "Md5Dialog"),
...
def main():
    ...
    import Super_ADB_Main  # noqa: F401
    s2 = measure("S2 +import Super_ADB_Main")
    ...
    if loaded_early:
        ...
        print("✓ 延迟 import 生效：import Super_ADB_Main 后 11 个重型子模块均未被加载")
    print("-" * 72)
