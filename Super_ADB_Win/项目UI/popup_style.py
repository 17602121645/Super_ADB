...
def add_green_glow(widget, blur_radius=24, alpha=200, accent=None):
    ...
    内部样式 (``setStyleSheet``) 由 ``Super_ADB_Win._propagate_theme_to_dialogs``
...
def _post_glow_kick(widget):
    ...
    popup_style 是因为弹窗（card）也需要这套 kick，主窗口调它时复用。"""
    try:
        ...
        if sys.platform == 'win32':
            try:
                ...
                if hwnd:
                    # LWA_ALPHA = 0x02
                    ctypes.windll.user32.SetLayeredWindowAttributes(hwnd, 0, 254, 0x02)
                    ctypes.windll.user32.SetLayeredWindowAttributes(hwnd, 0, 255, 0x02)
            except Exception:
                pass
        # ── 第 3 拳：延迟再 nudge（兜底事件循环延迟） ──
        QTimer.singleShot(150, lambda: _post_glow_kick_secondary(widget))
    except Exception:
        pass
