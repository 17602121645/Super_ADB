...
def 生成功能介绍Markdown():
    """生成「功能介绍与使用说明」Markdown 版本（仅功能介绍节，图片用相对路径）。"""
    features = [
        ...
        ("🌐 网络抓包", "项目说明/截图/网络抓包.png",
         ...
         ["点击「网络抓包」打开窗口，自动检测设备是否已安装 tcpdump",
          ...
          "点击「停止」自动拉取 pcap 文件并进入解析界面",
          "解析界面可查看每个数据包的详细信息，支持 HTTP 请求/响应查看"]),
        ...
        ("🛠️ 便捷工具", "项目说明/截图/便捷工具.png",
         ...
         ["**命令行**：一键打开系统终端，自动切换到项目目录",
          "**JSON 工具**：粘贴 JSON 自动格式化，支持左右对比差异，树形视图展开",
          "**哈希校验**：拖拽文件即算，支持多算法同时计算，可注册 Windows 右键菜单",
          "**时间戳转换**：输入时间戳自动识别单位并转换为北京时间，双向互转",
          "**ADB 终端**：交互式 adb shell，支持命令历史和自动补全",
          "**设备信息**：一键查看设备型号、Android 版本、序列号、屏幕分辨率、IP 地址等"]),
    ]
    ...
    lines.append("最新版本下载：")
    lines.append("")
    lines.append("- **夸克网盘**：https://pan.quark.cn/s/2b7b11ebe1e5?pwd=fAXN")
    lines.append("")
    lines.append("> 提取码：fAXN")
    lines.append("")
