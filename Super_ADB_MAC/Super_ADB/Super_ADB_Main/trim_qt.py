...
def _trim_windows(internal):
    ...
    if not os.path.isdir(ps):
        print('未找到', ps, '（先跑 pyinstall_y.py）')
        return
...
def main():
    ...
    # macOS: 优先走 .app 分支（即使 onedir 同时生成了 dist/Super_ADB/_internal）
    if sys.platform == 'darwin' and os.path.isdir(app_contents):
        _trim_mac(app_contents)
    elif os.path.isdir(internal):
        _trim_windows(internal)
    elif os.path.isdir(app_contents):
        _trim_mac(app_contents)
    else:
        print('未找到构建产物 dist/Super_ADB/_internal 或 dist/Super_ADB.app（先跑 pyinstall_y.py）')
        return
