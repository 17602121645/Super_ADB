...
class MainWindow(QWidget, Ui_MainWindow):
    ...
    def _send_text_via_native_clipboard(self, serial, text):
        """通过系统剪贴板 + 设备粘贴键, 实现免安装中文输入。

        策略:
        - Windows: 用 Win32 API (ctypes, 非 Qt) 设置剪贴板
          — 更可靠地触发剪贴板变更通知, 让模拟器同步到设备端
        - macOS/Linux: 用 Qt clipboard.setText() (跨平台)
        2. 等待模拟器剪贴板同步 (1.5s)
        ...
        try:
            ...
            if sys.platform == 'win32':
                # Windows: 用 Win32 API 写剪贴板（更可靠地触发变更通知）
                import ctypes
                ...
                # macOS/Linux: 用 Qt clipboard（跨平台）
...
def main():
    ...
    # macOS 兼容（2026-08-13）：PySide6 6.11.1 在 macOS 上，应用级事件过滤器会被
    # 传入已析构对象的悬挂指针 / 非 QObject 布局项（QSpacerItem），在 C++ 包装层
    # getWrapperForQObject 处段错误，启动数秒后 SIGSEGV。中文右键菜单在 macOS 上
    # 由 qtbase_zh_CN / qt_zh_CN 翻译器覆盖，故该过滤器仅在 Windows 安装。
    ...
    class _ZhContextMenuFilter(QObject):
        ...
        def eventFilter(self, obj, event):
            # 兼容性守卫：PySide6 6.11 + macOS 下，应用级事件过滤器可能收到
            # 非 QObject 对象（如 QSpacerItem）的事件；直接放行，防止
            # super().eventFilter() 因参数类型错误抛异常并递归崩溃。
