...
def install(main):
    ...
    # macOS: 捆绑 platform-tools（含 adb 二进制），让 .app 开箱即用
    extra_data = ''
    if sys.platform == 'darwin':
        pt_dir = os.path.join(os.path.dirname(here), 'platform-tools')
        if os.path.isdir(pt_dir):
            extra_data = f' --add-data "{pt_dir}{add_data_sep}platform-tools"'
            print(f'[macOS] 捆绑 platform-tools: {pt_dir}')
        else:
            print('[macOS] 警告: 未找到 platform-tools 目录，打包后 adb 将依赖系统 PATH')
    ...
    if sys.platform == 'darwin':
        ...
        icon = os.path.join(here, 'Super_ADB.icns') if os.path.exists(os.path.join(here, 'Super_ADB.icns')) else os.path.join(here, 'Super_ADB.png')
        cmd = f'pyinstaller --clean -w -i "{icon}" -n {name} {hidden} {hooks} {runtime_hooks} {excludes} {res_arg}{extra_data} {path_args} "{main}"'
    else:
        # Windows: 生成 .exe
