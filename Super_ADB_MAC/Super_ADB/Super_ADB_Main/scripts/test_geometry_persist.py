...
from adb_utils import load_json_config, save_json_config  # noqa: E402

CONFIG_NAME = "geo_test_config.json"  # 独立临时配置，绝不污染真实 adb_shell_config.json
...
class GeometryPersistTest(unittest.TestCase):
    ...
    def tearDownClass(cls):
        # 清理独立临时配置文件，避免残留
        try:
            os.remove(os.path.join(MAIN, CONFIG_NAME))
        except OSError:
            pass
