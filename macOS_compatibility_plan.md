...
| 10 | 性能监控（设备/应用） | `device_perf_monitor.py` / `app_perf_monitor.py` | ✅ | `dumpsys` 跨平台；`utils/chart_js.py` 内联图表库避免 CDN |
| 11 | WiFi 配对 / 扫码连接 | `wifi_pair_dialog.py` / `qr_connect_page.py` | ✅ | 二维码走 `pyzbar`（需 zbar，见 C6） |
...
| 17 | **本机 WiFi 密码查看** | `wifi_utils.py` + `wifi_dialog.py` | ❌ 不可用 | 依赖 Windows `netsh wlan`；macOS 无此命令；`diagnose()` 已返回"不支持"，但主功能仍会抛 `RuntimeError` |
| 18 | **计算哈希 + 右键菜单** | `md5_dialog.py` + `hash_context_menu.py` | ❌ **硬崩** | `md5_dialog.py:27` **仍为无条件 `import winreg`**（⚠️ 截至 2026-08-20 未修，仍是 P0 阻断） |
...
| 21 | **WiFi 密码审计** | `utils/wifi_pwd_cracker.py` | ⚠️ 仅 recover/crypt 不可用 | `crack` 是 WPA PMKID 模式，跨平台；`recover` 走 netsh，恢复本机已存 WiFi 密码仅 Windows |
...
#### B1. `md5_dialog.py` 顶部无条件 `import winreg`（**仍为硬崩溃**）

- **现象**：`md5_dialog.py:27` 写的是 `import winreg`（无 try/except 守卫），macOS 上只要打开「计算哈希 / MD5」功能（懒加载 `md5_dialog`），`import winreg` 失败 → 整个功能崩溃。
...
- **状态**：未实现，已在 `wifi_utils.diagnose()` 返回「不支持」时给用户友好提示。
...
| # | 任务 | 落地状态 | 修复细节 |
|---|------|---------|---------|
| ~~B1~~ | ~~`md5_dialog.py` 守卫 `import winreg`~~ | ❌ **未修复** | macOS 上点 MD5 按钮仍会硬崩 |
...
- **代码改动量小**：真正阻断的只有 B1（`md5_dialog` 守卫）一处；B3（B2 已修）需 macOS 真机做的体力活（拷 platform-tools），代码改动参考 `scrcpy()` 的探测模式即可。预计 **1–2 天**代码改动。
- **最大风险仍在外部依赖**：
  1. macOS 真机构建机（**目前缺失**）；
...
1. **修 `md5_dialog.py` 的 `import winreg` 守卫**（10 行代码，1 个文件）；
2. **在 macOS 真机构建机上**：
