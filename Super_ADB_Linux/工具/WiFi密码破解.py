...
def cmd_recover(args):
    ...
    - Windows：通过 netsh wlan 读取（无需管理员权限）
    - Linux：通过 NetworkManager (nmcli) 读取（可能需要 root/polkit 授权）
    - macOS：通过 security 钥匙串读取（需钥匙串授权）
    """
    if sys.platform != 'win32':
        # 非 Windows 平台复用 WiFi工具.py 的跨平台实现
        try:
            import importlib.util
            _wifi_utils_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'WiFi工具.py')
            spec = importlib.util.spec_from_file_location('wifi_utils', _wifi_utils_path)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            data = mod.collect_all()
        except Exception as e:
            print(f'[!] 读取 WiFi 配置失败: {e}')
            return 2
        if not data:
            print('[!] 未发现已保存的 WiFi 配置文件')
            return 1
        print(f'[+] 本机已保存 WiFi 配置: {len(data)} 个\n')
        rows = []
        for idx, d in enumerate(data, 1):
            pwd = d.get('password') or ''
            note = d.get('reason') or ''
            print(f'  {idx:2d}. {d["ssid"]:<24} 密码: {pwd or "(无/企业认证或隐藏)"}  {note}')
            rows.append((d['ssid'], pwd))
        if args.csv:
